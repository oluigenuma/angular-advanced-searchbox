        ...
        controller: [
          ...
          function($scope, $attrs, $element, $timeout, $filter, setFocusFor) {
            ...
            $scope.$watch('model', function(newValue, oldValue) {
              ...
              angular.forEach($scope.model, function(value, key) {
                if (key === 'query' && $scope.searchQuery !== value) {
                  ...
                  var paramTemplate = $filter('filter')($scope.parameters, function(param) {
                    return param.key === key; })[0];
                  var searchParams = $filter('filter')($scope.searchParams, function(param) {
                  ...
                  if (paramTemplate !== undefined) {
                    if (paramTemplate.allowMultiple) {
                      ...
                      value.forEach(function(val, valIndex) {
                        if (searchParams.some(function(param) {
                            return param.index === valIndex; })) {
                          var param = searchParams.filter(function(param) {
                            return param.index === valIndex; });
              ...
              //     var index = $scope.searchParams.map(function(e) {
              //       return e.key; }).indexOf(value.key);
            ...
            $scope.isUnsedParameter = function(value, index) {
              return $filter('filter')($scope.searchParams, function(param) {
                return param.key === value.key && !param.allowMultiple; }).length === 0;
            };

            $scope.toggleShowSearchParams = function() {
              $scope.showSearchParams = !$scope.showSearchParams;
            ...
            $scope.addSearchParam = function(searchParam, value, enterEditModel) {
              ...
              if (searchParam.allowMultiple)
                internalIndex = $filter('filter')($scope.searchParams, function(param) {
                  return param.key === searchParam.key; }).length;
            ...
            $scope.removeSearchParam = function(index) {
              ...
              if (searchParam.allowMultiple) {
                var paramsOfSameKey = $filter('filter')($scope.searchParams, function(param) {
            ...
            function restoreModel() {
              angular.forEach($scope.model, function(value, key) {
                if (key === 'query') {
                  ...
                  var searchParam = $filter('filter')($scope.parameters, function(param) {
                    return param.key === key; })[0];
            ...
            function updateModel(command, key, index, value) {
              ...
              changeBuffer = $filter('filter')(changeBuffer, function(change) {
                return change.key !== key && change.index !== index; });
...
angular.module('angular-advanced-searchbox').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('angular-advanced-searchbox.html',
    "<div class=advancedSearchBox ng-class={active:focus} ng-init=\"focus = false\" ng-click=\"!focus ? setFocusFor('searchbox') : null\"><span ng-show=\"searchParams.length < 1 && searchQuery.length === 0\" class=\"search-icon glyphicon glyphicon-search\"></span> <a ng-href=\"\" ng-show=\"searchParams.length > 0 || searchQuery.length > 0\" ng-click=removeAll() role=button><span class=\"remove-all-icon glyphicon glyphicon-trash\"></span></a><div><div class=search-parameter ng-repeat=\"searchParam in searchParams\"><a ng-href=\"\" ng-click=removeSearchParam($index) role=button><span class=\"remove glyphicon glyphicon-trash\"></span></a><div class=key data-key={{searchParam.key}} ng-click=\"enterEditMode($event, $index)\">{{searchParam.name}}:</div><div class=value><span ng-show=!searchParam.editMode ng-click=\"enterEditMode($event, $index)\">{{searchParam.value}}</span> <input name=value nit-auto-size-input type={{searchParam.type}} set-focus-on=\"{{'searchParam:' + searchParam.key}}\" ng-keydown=\"keydown($event, $index)\" ng-blur=\"leaveEditMode($event, $index)\" ng-show=searchParam.editMode ng-change=\"searchParam.restrictToSuggestedValues !== true ? searchParamValueChanged(searchParam) : null\" ng-model=searchParam.value uib-typeahead=\"suggestedValue for suggestedValue in searchParam.suggestedValues | filter:$viewValue\" typeahead-min-length=0 typeahead-on-select=\"searchParamTypeaheadOnSelect($item, searchParam)\" typeahead-editable=\"searchParam.restrictToSuggestedValues !== true\" typeahead-select-on-exact=true typeahead-select-on-blur=\"searchParam.restrictToSuggestedValues !== true ? false : true\" placeholder=\"{{searchParam.placeholder}}\"></div></div><input name=searchbox nit-auto-size-input class=search-parameter-input set-focus-on=searchbox ng-keydown=keydown($event) placeholder={{placeholder}} ng-focus=\"focus = true\" ng-blur=\"focus = false\" uib-typeahead=\"parameter as parameter.name for parameter in parameters | filter:isUnsedParameter | filter:{name:$viewValue} | limitTo:parametersDisplayLimit\" typeahead-on-select=\"searchQueryTypeaheadOnSelect($item, $model, $label)\" ng-change=searchQueryChanged(searchQuery) ng-model=\"searchQuery\"></div><div class=search-parameter-suggestions ng-show=\"parameters && focus\"><span class=title>{{parametersLabel}}:</span> <span class=search-parameter style=background-color:#5bc0de ng-mousedown=toggleShowSearchParams()>{{showSearchParams?'HIDE':'SHOW'}}</span> <span class=search-parameter ng-repeat=\"param in parameters | filter:isUnsedParameter | limitTo:parametersDisplayLimit\" data-key={{param.key}} ng-mousedown=addSearchParam(param) ng-show=showSearchParams>{{param.name}} <i ng-class=\"{'glyphicon glyphicon-plus': param.allowMultiple}\"></i></span></div><div ng-repeat=\"res in results\" class=mt-dropdown-content ng-show=focus><a ng-mousedown=select(res)><span><p class=pull-left>{{res.name}}</p><p ng-if=res.type style=font-size:10px>Type: {{res.type | convertType}}</p></span></a></div></div>"
  );
